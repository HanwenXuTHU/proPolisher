...
class Generator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=512, seqL=50, bias=True):
        ...
        self.lstm = nn.LSTM(input_size=ngf, hidden_size=int(ngf/2),
...
def main():
    ...
    n_critics = 5
    n_iters = 10000
    model = WGAN(input_nc=4, output_nc=4, seqL=100, l1_w=100)
    ...
    for i in range(n_iters):
        ...
        if i % 100 == 99:
            ...
            logger.info('polyT ref TTTTT:{} TTTTTT:{} TTTTTTT:{} TTTTTTTT:{}'.format(T_dict_ref['TTTTT'],
                                                                               ...
                                                                               T_dict_ref['TTTTTTT'],
                                                                               T_dict_ref['TTTTTTTT']))
            csv2fasta(csv_name, 'cache/reserve/gen_', 'iter_{}'.format(i))
            torch.save(model.generator, 'cache/reserve/' + 'net_G_' + str(i) + '.pth')
            torch.save(model.discriminator, 'cache/reserve/' + 'net_D_' + str(i) + '.pth')
