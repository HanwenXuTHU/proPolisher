...
def main():
    train_data, test_data = DataLoader(LoadData(is_train=True), batch_size=32, shuffle=True), DataLoader(LoadData(is_train=False), batch_size=32)
    train_data = get_infinite_batches(train_data)
    n_critics = 5
    n_iters = 20000
    model = WGAN(input_nc=4, output_nc=4, seqL=100)
    ...
    print('Saving Test Succeed!')
    torch.save(model.generator, 'check_points/' + 'G_' + str(0) + '.pth')
    torch.save(model.discriminator, 'check_points/' + 'D_' + str(0) + '.pth')
    print('Saving Model Test Succeed!')
        ...
        if i % 100 == 99:
            ...
            for j, eval_data in enumerate(test_data):
                with torch.no_grad():
                    ...
                    tensorInput.append(eval_data['in'])
                    tensorRealB.append(eval_data['out'])
            print('Training: iters: {}, dloss: {}, gloss:{}, gl1:{}'.format(i, d_loss / 100 / n_critics, g_loss / 100, g_l1 / 100))
            print('Testing: reserve percentage: {}%'.format(reserve_percentage(tensorInput, tensorSeq)))
            d_loss, g_loss, g_l1 = 0, 0, 0
            csv_name = save_sequence(tensorSeq, tensorInput, tensorRealB, 'wgan_')
            csv2fasta(csv_name, 'cache/gen_', 'iter_{}'.format(i))
            torch.save(model.generator, 'check_points/' + 'G_' + str(i) + '.pth')
            torch.save(model.discriminator, 'check_points/' + 'D_' + str(i) + '.pth')
