...
class LoadData(Dataset):
    ...
    def __init__(self, path='data/ecoli_100_reserve-35.csv', split_r=0.9, is_train=True, gpu_ids='0'):
        realB = list(pd.read_csv(path)['output'])
        realA = list(pd.read_csv(path)['input'])
        data_size = len(realB)
